...
function initDatabase() {
  ...
  const schemaSQL = `
    ...
    -- Stakeholders / Intervenants table
...
function insertDemoData(db, callback) {
  const bcrypt = require('bcryptjs');
  const password = bcrypt.hashSync('admin123', 10);
  
  const demoData = `
    INSERT OR IGNORE INTO users (email, password_hash, first_name, last_name, role)
    VALUES ('admin@terrazza.com', '${password}', 'Admin', 'Terrazza', 'admin');
    ...
    VALUES 
      ('Discovery & Requirements', 'Define stakeholder roles, property specs, and luxury service tiers.', 'Phase 1: Foundation', 'completed', '2024-01-01', '2024-01-31'),
      ('Brand Identity & Design', 'Create visual identity, mood boards, color palette, and design system.', 'Phase 1: Foundation', 'completed', '2024-02-01', '2024-02-28'),
      ...
      ('Integration & Testing', 'Connect all stakeholder workflows, test booking flow, validate payment placeholder.', 'Phase 2: Build', 'pending', '2024-06-01', '2024-06-30'),
  ...
  db.exec(demoData, (err) => {
    if (err) console.error('Demo data error:', err);
    callback();
  });
}
